...
def make_learning_rate_decay_fn(opt):
    ...
    if opt.decay_method == 'noam':
        ...
        return functools.partial(
            linear_decay,
            warmup_steps=opt.warmup_steps,
            warmup_end_lr=opt.learning_rate,
...
def linear_decay(step,  warmup_steps, warmup_end_lr, total_steps, min_lr, warmup_init_lr=-1):
    max_steps = total_steps - warmup_steps
    if warmup_init_lr < 0:
        warmup_init_lr = warmup_end_lr
    if step < warmup_steps:
        return (warmup_init_lr + step * (warmup_end_lr - warmup_init_lr) / warmup_steps)/warmup_end_lr
    else:
        return (warmup_end_lr * (1.0 - (step - warmup_steps) / max_steps) + min_lr)/warmup_end_lr
...
def noam_decay(step, warmup_steps, model_size):
    ...
    """
    return (model_size ** (-0.5) * min(int(step//5 + 1) ** (-0.5), int(step//5 + 1)*warmup_steps**(-1.5), 0.016))
    #(model_size ** (-0.5) * min(int(step//2 + 1) ** (-0.5), int(step//2 + 1) * warmup_steps**(-1.5)))
...
class RAdam(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.0001, degenerated_to_sgd=True):
        if not 0.0 <= lr:
