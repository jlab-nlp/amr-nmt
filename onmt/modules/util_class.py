...
class ElementwiseModule(nn.Module):
    """
    A simple network container.
    Parameters are a list of modules.
    Inputs are a 3d Tensor whose last dimension is the same length
    ...
    def __init__(self, embedding):
        super(ElementwiseModule, self).__init__()
        self.embedding = embedding

    def forward(self, seq_input):
        seq_input = torch.squeeze(seq_input, dim=-1)
        reshaped_seq_input = seq_input.transpose(0, 1).contiguous()
        out_put = self.embedding(reshaped_seq_input)
        reshaped_output = out_put.transpose(0, 1).contiguous()
