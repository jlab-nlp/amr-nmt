...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, head_count, model_dim, dropout=0.1,
                 ...
                 fusion="cat", gate=False, use_full=True):
        ...
        if edge_type > 0:
            ...
            self.linear_attention = nn.ModuleList(
                [nn.Linear(self.dim_per_head, 1)
                 for _ in range(2 * (edge_type+1))] # three edge-aware sub graph and one the whole graph
            )
            self.fusion = fusion
            #use_full = False
            if use_full:
                ...
                self.sub_final_linear = nn.Linear(model_dim * (edge_type), model_dim)
    ...
    def forward(self, key, value, query, grh=None, mask=None,
        ...
        if grh is not None:
            ...
            for i in range(self.edge_type-1):
                ...
                # eye = torch.zeros(grh.size(-1), grh.size(-1))
                # eye = eye.fill_diagonal_((4-label_id))
                eye = torch.eye(grh.size(-1), dtype=torch.int64, device=grh_mask.get_device()) * (4-label_id)   # here doesnt support multi-gpu
                # eye = torch.eye(grh.size(-1), dtype=torch.int64) * (4-label_id)
