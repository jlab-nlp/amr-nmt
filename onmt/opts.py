...
def model_opts(parser):
    ...
    group.add('--feat_merge', '-feat_merge', type=str, default="first",
              choices=['concat', 'sum', 'mlp', "first"],
    ...
    group.add('--model_kind', '-model_kind', default='vanilla',
              choices=['vanilla', 'concat','seperate','concat_type_encoding', 'mha_concat', 'mha_add_concat', 'concat_single_decoder', 'mha_concat_single_decoder', 'bilstm_gat'],
              help="Type of source model to use. Allows "
                   "the system to incorporate non-text inputs. "
                   "Options are [text|img|audio|vec].")
    ...
    group.add('--use_full', '-use_full', action='store_true',
              help="If using whole graph attention")
...
def preprocess_opts(parser):
    ...
    # NMT AMR options
    group = parser.add_argument_group('NMT_AMR')
    group.add('--train_mt_src', '-train_mt_src', nargs='+',
              help="Path to the training NMT AMR source data")
    group.add('--train_mt_tgt', '-train_mt_tgt', nargs='+',
              help="Path to the training NMT AMR target data")
    group.add('--valid_mt_src', '-valid_mt_src',
              help="Path to the validation NMT AMR source data")
    group.add('--valid_mt_tgt', '-valid_mt_tgt',
              help="Path to the validation NMT AMR target data")
    group.add('--src_lang', '-src_lang',
              help="source language type")
    group.add('--tgt_lang', '-tgt_lang',
              help="target language type")
    group.add('--bart_config', '-bart_config',
              help="bart model path or pretrained model type")
...
def train_opts(parser):
    ...
    group.add('--debug', '-debug', action='store_true',
              help="Set the training data and valid data as debug data")
    ...
    group.add('--optim', '-optim', default='sgd',
              choices=['sgd', 'adagrad', 'adadelta', 'adam',
                       'sparseadam', 'adafactor', 'fusedadam', 'radam'],
    ...
    group.add('--min_learning_rate', '-min_learning_rate', type=float, default=1e-9,
    ...
    group.add('--warmup_init_lr', '-warmup_init_lr', type=float, default=-1,
              help="Minimum learning rate. ")
    group = parser.add_argument_group('Logging')
...
def translate_opts(parser):
    ...
    # NMT AMR options
    group = parser.add_argument_group('NMT_AMR')
    group.add('--eval_mt_src', '-eval_mt_src', nargs='+',
              help="Path to the evaluating NMT AMR source data")
    group.add('--eval_mt_tgt', '-eval_mt_tgt', nargs='+',
              help="Path to the evaluatingNMT AMR target data")
    group.add('--src_lang', '-src_lang',
              help="source language type")
    group.add('--tgt_lang', '-tgt_lang',
              help="target language type")
