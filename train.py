...
def main(opt):
    ...
    logger.info("res.pad_token_id:", res.pad_token_id)
    logger.info("Finished loading ReduceEmbeddingSize!")
...
def batch_producer(generator_to_serve, queues, semaphore, opt):
    ...
    for device_id, q in cycle(enumerate(queues)):
        ...
        b.grh = b.grh.to(torch.device(device_id))
        input_ids = b.bart_text["input_ids"].to(torch.device(device_id))
        decoder_input_ids = b.bart_text["decoder_input_ids"].to(torch.device(device_id))
        labels = b.bart_text["labels"].to(torch.device(device_id))
        new_bart_text = {"input_ids": input_ids,
                         "decoder_input_ids": decoder_input_ids,
                         "labels": labels}
        b.bart_text = new_bart_text
