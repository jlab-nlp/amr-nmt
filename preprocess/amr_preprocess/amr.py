...
def clean_grammar_file(s):
    ...
    return re.sub('\n[ \t]+', ' ', s.replace('\t', ' ').replace('`', '_backtick'))

with open('preprocess/amr_preprocess/amr_test.peg') as inF:
...
class AMR(DependencyGraph):
    ...
    def __init__(self, anno, tokens=None):
        ...
        if anno:
            new_anno = self.fix_parentheses(self.fix_others(self.fix_concept(self.detect_unformed_variable_and_replace(anno))))
            self._anno = new_anno
            msg = ''
            try:
                p = grammar.parse(new_anno)
    ...
    def detect_unformed_variable_and_replace(self, amr):
        result = re.findall("\(-?[0-9]*\s*/|\(-\s*/|\(\.\s*/|\('\s*/|\([A-Z]*[0-9]*\s*/|\([^\x00-\x7f]\s*/", amr)
        new_amr = amr
        new_num = 0
        for n in range(len(result)):
            assert f"(z{str(n)} /" not in new_amr, f"(z{str(n)} /" + " [in] " + new_amr
        for matched in result:
            new_amr = new_amr.replace(matched, f"(z{str(new_num)} /", 1)
            new_num += 1
        return new_amr

    def fix_concept(self, amr):
        result = re.findall("/ \"[^ \n\t\"]*\"", amr)
        new_amr = amr
        for match in result:
            new_match = match
            if "(" in new_match:
                ...
                new_amr = new_amr.replace(match, new_match, 1)
            if ")" in new_match:
                old_match = new_match
                new_match = old_match.replace(")", "")
                new_amr = new_amr.replace(old_match, new_match, 1)
            old_match = new_match
            new_match = old_match.replace("\"", "")
            new_amr = new_amr.replace(old_match, new_match, 1)
        result = re.findall("/ [^ \n\t\"]*", new_amr)
    ...
    def fix_others(self, amr):
        new_amr = amr
        if "\"(\"" in new_amr:
            new_amr = new_amr.replace("\"(\"", "-")
        if "\")\"" in new_amr:
            new_amr = new_amr.replace("\")\"", "-")
        return new_amr
