        ...
        # we replace the *entire tuple* with an anonymization token.
            ...
            for triple in triples:
                if triple[0] == conc and triple[1] == ':instance-of':
                    if triple not in output_triples:
                        output_triples.append((triple[0], triple[1], Concept(new_quant_name)))
                    else:
                        output_t_index = output_triples.index(triple)
...
def get_line_graph(graph, surf, anon=False, scope=False):
    ...
    if scope and nodes_to_print != [] and top_node in nodes_to_print:
    ...
    # Fix the problem of space within a node, use _ to connect to make it as one phrase
    new_nodes_to_print = []
    for node_to_print in nodes_to_print:
        sub_node = node_to_print.split(" ")
        if len(sub_node) > 1:
            sub_node = "_".join(sub_node)
        else:
            sub_node = sub_node[0]
        new_nodes_to_print.append(sub_node)
    return new_nodes_to_print, triples, anon_surf, anon_map, nodes_scope
